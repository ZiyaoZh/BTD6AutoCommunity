...
namespace BTD6AutoCommunity
{
    public class CirculationStrategy
    {
        ...
        {
            MapTypes mapType = GetMapType(currentMap);
            Point mapTypePos = GetMapTypePos(mapType);
            Point mapPos = GetMapPos(_context, (int)currentMap);
            int reTryCount = 0;
            while (mapPos.X == -1)
            {
                if (reTryCount > 8)
                ...
                Thread.Sleep(500);
                mapPos = GetMapPos(_context, (int)currentMap);
            }
            MouseMoveAndLeftClick(_context, mapPos.X, mapPos.Y);
            IsMapSelectionComplete = true;
            _logs.Log($"已选择地图：{GetTypeName(currentMap)}", LogLevel.Info);
        }
        ...
        {
            MouseMoveAndLeftClick(_context, 1080, 840);
            Thread.Sleep(500);
            MouseMoveAndLeftClick(_context, 1135, 730);
        }
